...
public class _11_Excel {
	...
	try (FileInputStream input = new FileInputStream("src/11. Incomes-Report.xlsx")) {
	    XSSFWorkbook workbook = new XSSFWorkbook(input);
	    XSSFSheet sheet = workbook.getSheetAt(0);
	    ArrayList<_11_Office> offices = new ArrayList<>();
	    for (int i = 1; i < 12; i++) {
		XSSFRow currentRow = sheet.getRow(i);
		_11_Office office = new _11_Office(currentRow.getCell(0)
			.getStringCellValue(), currentRow.getCell(5)
			.getNumericCellValue());
		offices.add(office);
	    }
	    ArrayList<_11_Office> officesTotal = new ArrayList<>();
		...
		for (int j = i + 1; j < offices.size(); j++) {
			...
			officesTotal.add(new _11_Office(offices.get(i).getTown(),
				total));
	    ...
	    for (_11_Office office : officesTotal) {
